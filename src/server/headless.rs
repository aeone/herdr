...
fn apply_terminal_attach_input(
    ...
    let wants_mouse = runtime.mouse_reporting_enabled();
...
impl HeadlessServer {
    ...
    fn pty_render_targets(&self) -> (bool, HashSet<&str>) {
            ...
            .filter(|client| client.writer.is_some())
        {
            match &client.mode {
                ...
                // A watcher of many terminals is a render target for every one
                // of them; leaving them out stops mirrors being drawn at all.
                ClientConnectionMode::TerminalObserveMany => {
                    for observed in &client.observed {
                        direct_terminal_targets.insert(observed.terminal_id.as_str());
                    }
                }
                ClientConnectionMode::App => {}
            }
    ...
    fn refresh_watched_for_someone(&mut self) {
        // Only the unix arm below adds to it.
        ...
        // Unix only, as the rest of mirroring is: the runtime registry only
        // offers a walk of its terminals there.
        #[cfg(unix)]
...
"#,
    ...
    /// The test client writer drains its queue on a background thread, so a
    /// message pushed by a render has to be waited for rather than tried once.
    fn next_observed_terminal_ended(
        control_rx: &std::sync::mpsc::Receiver<Vec<u8>>,
        ...
        while let Some(remaining) = deadline.checked_duration_since(Instant::now()) {
            ...
            if matches!(message, ServerMessage::ObservedTerminalEnded { .. }) {
                return Some(message);
            }
        }
        None
    ...
    fn one_target_that_has_gone_does_not_cost_the_watcher_the_others() {
        with_terminal_session_test_server(|server, _terminal_id, terminal_id_string, _| {
            ...
            let ended = next_observed_terminal_ended(&control_rx);
            assert!(
                matches!(
                    ended,
