...
row_gap = 0
rows = [
  ["state_icon", "remote_host", "workspace", "tab"],
  ["agent"],
]
...
row_gap = 0
rows = [
  ["state_icon", "remote_host", "workspace"],
  ["branch", "git_status"],
]
```
...
- `number` — 1-9 switch position of this agent, blank past 9; matches the `focus_agent` indexed keybind.
- `workspace` — workspace name.
- `remote_host` — host prefix for an agent mirrored from a remote Herdr; empty for local agents.
...
- `number` — 1-9 switch position of this space, blank past 9; matches the `switch_workspace` indexed keybind.
- `workspace` — workspace name.
- `remote_host` — host prefix for a space mirrored from a remote Herdr; empty for local spaces.
...
`remote_host` renders dimmed by default so it labels where a row came from without competing with the name beside it. Give it an inline `fg` to pick your own:

```toml
[ui.sidebar.spaces]
rows = [["state_icon", { token = "remote_host", fg = "#f38ba8", bold = true }, "workspace"]]
```
...
### Sizing the Space and Agent sections

The sidebar divider sits where you drag it. Set `sidebar_section_split_auto` to size the two sections from their content instead:

```toml
[ui]
sidebar_section_split_auto = true
```

When both lists fit, nothing changes. When one overflows, the section that fits is given exactly what it needs and the overflowing one takes the rest, so a short Space list does not hold back a long Agent list. Each section keeps a minimum height so neither header disappears. The dragged position is remembered and used again when this is `false`.

### Hiding spaces the Agent panel already lists

A space that hosts agents appears in both sidebar sections. Set `hide_when_in_agents` to list it only under Agents:
...
It defaults to `false`. Only spaces with at least one pane in the Agent panel are hidden, so plain shell spaces stay listed. The active space always stays visible so the current space never disappears from the Space list. Keyboard and mouse space navigation follow the visible list, so hidden spaces are skipped; they remain reachable through the Agent panel. When a hidden space is the parent of a worktree group, the first remaining child takes its place as the group header.
