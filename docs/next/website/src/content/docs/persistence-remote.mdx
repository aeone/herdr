...
## Mixing remote spaces into the local sidebar

`herdr --remote` replaces your whole session with the remote one. To instead see a remote host's agents *beside* your local ones, list the host under `[[remote.spaces]]`:

```toml
[[remote.spaces]]
target = "workbox"       # ssh target, same syntax as --remote
...
mirror_all = false       # mirror every space, not only those with an agent
color = "blue"           # colour for this host's sidebar prefix
```

Herdr polls each configured host over SSH, asks the remote Herdr for its panes, and adds one local space per remote *agent* pane. Selecting one attaches to that remote pane, so the agent is fully interactive.

The host is shown by the `remote_host` sidebar token rather than baked into the space name. Each host takes a stable colour derived from its name; set `color` on a host to choose one explicitly (any theme-colour value). See [sidebar row layouts](/docs/configuration/#sidebar-row-layouts).

To mirror another Herdr session on the same machine, set `target = "local"` and name the session. This needs no SSH access at all:
...
Herdr refuses a local entry that points at the session it is running in, since each mirror pane is itself an agent pane and it would mirror its own mirrors without bound.

Repeat the block for as many hosts as you want. Each host is polled independently.

Notes on what gets mirrored and when:

- By default only panes the remote recognizes as agents are mirrored, so plain shells on the host stay out of your sidebar. Set `mirror_all = true` on a host to mirror every space instead; agent-less spaces are mirrored by their first pane. This is what to use when you want the host's whole sidebar, not just its agents, but it opens one SSH attach per mirrored space.
- Mirrored spaces are derived state. They are never written to `session.json`, and they reappear by polling after a restart rather than being restored.
- Mirrors appear without stealing focus, and are removed when their remote pane goes away.
- When a poll fails, existing mirrors are left in place and the host is retried on the next interval, so a brief network drop does not clear and repopulate your sidebar.
- When a mirror's own connection dies, the mirror is removed. Set `[remote].keep_offline_mirrors = true` to keep it instead, greyed and sorted to the bottom, until the host answers again and the pane is rebuilt — worth it for a fleet with a laptop that sleeps, where the list otherwise shrinks and grows under you. The `toggle_offline_mirrors` keybind flips this for the running session, and that answer is kept with the session rather than reset by a config reload.
- Remote space names are not unique. When several mirrored spaces would share a name, Herdr appends a number.
- Each host needs SSH access and a Herdr server already running there. Herdr looks for `herdr` on the remote `PATH`, then in `~/.local/bin`, `~/.cargo/bin`, `/usr/local/bin`, and `/opt/homebrew/bin`. Unlike `--remote`, it never installs a remote binary.

Mirroring is Unix-only, matching remote attach.
