...
Herdr polls each configured host over SSH, asks the remote Herdr for its panes, and adds one local space per remote *agent* pane, named `<label>/<remote space>`. Selecting one attaches to that remote pane, so the agent is fully interactive.
