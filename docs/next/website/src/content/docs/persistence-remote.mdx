...
The host is shown by the `remote_host` sidebar token rather than baked into the space name, so you can colour it separately from the workspace name. See [sidebar row layouts](/docs/configuration/#sidebar-row-layouts).
