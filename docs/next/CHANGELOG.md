...
### Fixed
- The size a watcher asks for is the size the watched terminal becomes. A terminal is rendered at its own size, so a mirror in a taller pane got the remote's shorter screen painted into the top and blank below -- a 23-row host in a 70-row pane filling a third of it -- and a taller one was clipped. Asking is what sizes it now, as it was when a mirror was a whole attach: the machine being used wins, and stops winning when it stops asking, so a session driven from a phone is phone-sized only while the phone is the thing being used. Only naming a set resizes anything, never a render, so two watchers of one terminal take turns rather than fight. This chains: a mirror reached through another host carries the size down each hop.
