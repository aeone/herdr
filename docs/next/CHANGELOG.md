...
### Added
- A remote space is mirrored as a space, holding a tab per pane, rather than as one local space per remote pane. A remote space with several panes used to arrive as several unrelated local spaces, told apart only by an ordinal stuck on the end of a shared name, and a plain shell sharing a space with an agent did not arrive at all. A tab asked for on a mirror came back as yet another space rather than a tab, which is why asking for one looked like nothing had happened. Mirrors are now identified by the space they stand for and their tabs by the panes in it, so a pane appearing is a tab opening and a pane going is a tab closing, while the space stays put. Every tab is watched and sized at the size it is drawn at, not just the first.
...
- Switching a host's mirrors off no longer makes its panes arrive the long way round. The fleet is a star -- a leaf is mirrored by the hub, and a machine mirroring the hub sees every leaf pane twice, once straight from the leaf and once reflected through the hub. The reflection is dropped because the leaf is a host we mirror, and switching the leaf off took it out of that set, so the reflection stopped being recognised and came back: the same panes, two hops away, under the hub's spelling of them, appearing and vanishing with the leaf's own sleep. Off now means that machine's panes are not wanted however they get here.

- A mirror's writable connection is let go when the pane holding it closes, and dropped when a write to it fails. That connection is an attach, and an attach sizes the terminal it lands on, so both faults ended in the same place: the host resizing a terminal for a viewer who was not there. A claim outlived the pane that made it, so switching one host's mirrors off in the keybind overlay -- or a host's own handoff rebuilding its mirrors -- left another mirror attached to a terminal nothing was showing. And a connection that had died on the far side was kept, so every keystroke after the first fault claimed afresh: attach, size, fail, attach, dropping the keystroke each time. Typing into a mirror after any hiccup was a resize storm on the other machine. A failed write now means the connection is gone, and the keystroke that found the fault is sent on a fresh one rather than eaten.

- Typing into a mirror no longer resizes the terminal behind it to the wrong size. The writable connection a mirror claims is an attach, and a host sizes a terminal to its attach client -- so claiming one resized it to whatever pty that ssh happened to get, undoing the size the watching path had just worked out. A 70-row pane typed into became a 40-row terminal on the other machine. The claim now says how big the pane is as the first thing it does, and says it again whenever it moves to another pane, since moving is another attach.

- A size that is still a guess is never asked for as a resize, and the guess itself is the size of the screen rather than 80x24. A mirror pane is created at a guess and asks its host to render at it, and a host now resizes to what it is asked -- so on a machine whose panes are 70 rows tall, every handoff that rebuilt the mirrors briefly asked their hosts for 24 rows, shrinking real terminals on the other machine and reflowing their output before the layout arrived and corrected it. A machine that has laid nothing out asks for no resize at all now, and once it has, the size is real.

- The size a watcher **that someone is looking at** asks for is the size the watched terminal becomes. A terminal is rendered at its own size, so a mirror in a taller pane got the remote's shorter screen painted into the top and blank below -- a 23-row host in a 70-row pane filling a third of it -- and a taller one was clipped. Asking is what sizes it now, as it was when a mirror was a whole attach: the machine being used wins, and stops winning when it stops asking, so a session driven from a phone is phone-sized only while the phone is the thing being used. Only naming a set resizes anything, never a render, so two watchers of one terminal take turns rather than fight. Only a watcher showing the terminal to someone asks, which matters more than it sounds: a hub holding mirrors nobody is looking at has no pane to measure and would otherwise impose its fallback guess on every terminal it watches. That answer travels, so a mirror reached through another host carries the size down each hop -- a hub is nobody on its own, but a hub being watched by a machine with someone at it is.

- A multiplexed mirror can be scrolled again. Scrollback belongs to the host: a mirror is fed rendered screens, so nothing ever rolls off into a scrollback of its own, and the wheel was moving a local viewport with nothing behind it. It now asks the host to move that terminal's viewport, over the same writable connection typing uses and by the same rules -- a pane reporting the mouse gets the wheel as mouse events, as a direct attach would. This worked before mirrors shared a connection, because the per-pane attach was a full client that forwarded scroll.
...
- A mirror is asked for at the size of the local terminal its frames are written into, taken from that terminal rather than from the laid-out view. The view is the wrong place to ask: it holds whichever attached client rendered last, and only ever for the workspace on screen, so a mirror could be asked for at a size nothing was expecting -- and asking again as the view moved rewrote the set for panes that had not changed at all. A screen rendered for the wrong width wraps wrongly wherever it lands, which is why a mirror in that state could not be read, could not be scrolled, and was not recognised as a working agent until something happened to send its true size. Every mirror of a host used to be requested at one guessed size: whichever pane happened to be first in the workspace on screen, measured to its outer edge rather than its content. The hosts are now told again after any render that resized a mirror pane, so activating one, zooming it, switching workspace or resizing the window each correct it immediately; a render that resized nothing costs one comparison. This was survivable while hosts re-rendered every watched terminal sixty times a second, because a fresh frame was never more than a moment away; once that stopped, a mirror drawn for the wrong shape simply stayed that way until something else disturbed it.

### Added
- Mirroring can be switched off and back on per host from the keybind overlay. Each configured machine gets a row at the top of the overlay, clickable or reachable by its number, and the switch beside the title moves all of them at once -- reading `mirrors 1/2` rather than claiming to be on when only some are. Off means this machine behaves as though that host were not configured: its mirror panes close, its feed stops and its connections are let go, so it is the honest way to drop a machine for a while rather than a way of hiding rows. The answers are kept with the session, so a deploy's live handoff does not quietly put a host back. The keys are live only while the overlay is open and only on a machine with `[[remote.spaces]]` configured, so they cost no global binding.
...
- A host too old to stream many terminals at once keeps an attach per mirrored pane, decided per host rather than per fleet: it prints its usage and exits, which looks like a dropped connection except that nothing ever came down it, so a connection that carried no frames at all is what marks the host. A connection that worked and then dropped keeps the shared one, since that says nothing about what the host can do. A host is old only until someone deploys to it, so it is asked again an hour later rather than written off for the life of the process. Mixed builds are the normal state of a fleet where some machines sleep for days.
- A multiplexed mirror can be typed into. Watching claims nothing, so the writable claim is taken on the first keystroke and moved from pane to pane over one connection per host, rather than one per pane or one per switch — a terminal takes one controller at a time, and over ssh a fresh connection is a process and a handshake in front of the keystroke that asked for it. A pane being laid out no longer claims anything, so only panes actually being typed into are held. Two mirrors of one host measured one watching connection and one writable one between them, whichever was being used.
- A connection that already names a terminal may name another: an observer whose set has changed because panes came and went on the host, and a controller following the pane in use. Both were refused before, which meant a watcher lost its stream the moment the host gained a pane.
- Added `remote.multiplexed_mirrors`, which gives every mirror of a host one shared connection instead of an ssh, a remote process, a PTY pair and an exclusive claim on the terminal each. A mirrored pane becomes a terminal fed by that stream rather than a local `ssh herdr terminal attach`, so nothing is spawned per pane and nothing is claimed: several machines can now mirror the same host at once, which the per-pane attach forbids and which is why the fleet had to mirror in a star. Two watchers of a two-pane host measured nine open descriptors each, against roughly fourteen per mirrored pane before. Off by default while it is being proven, and hosts too old to understand `terminal session observe-many` keep the per-pane attach.
- Added `herdr terminal session observe-many`, which watches any number of terminals over a single connection and prints their frames as NDJSON, each naming the terminal it belongs to. The terminals are named on stdin (`{"type":"terminal.observe","targets":[{"target":"w1:p1","cols":40,"rows":8}]}`) rather than in the command line, because the set changes as panes come and go and reopening the connection for each change is what this exists to avoid. An idle terminal costs nothing after its first frame.
- The server can now stream several terminals to one client connection: `ObserveTerminals` names a set of terminals and the size to render each at, and frames come back tagged with the terminal they belong to (`ObservedTerminal`, `ObservedTerminalEnded`). Mirroring one remote pane costs a whole ssh, a remote process, a connection and an exclusive claim on the terminal today, so a machine holding thirty mirrors holds thirty of each; this is what lets a watcher hold one connection per host instead. Frames for different terminals are queued per terminal rather than in the single render slot, since one terminal's frame is no substitute for another's. Protocol version 18 to 19; single-target `ObserveTerminal` is unchanged, so a host on an older build still mirrors the old way.
- The server hands over to a new build on `SIGUSR1`, the same live handoff `herdr update` performs, so a deploy no longer has to fall back on a signal that closes every pty and takes the agent processes with it.
- The keybind help lists fork-only actions first, under their own `ryi` heading, so they are not lost among the upstream bindings. Only bound actions appear and the heading is omitted when none are.
- `pane get` and `pane list` now count the wheel events herdr routed to each pane, split by destination, alongside how a pane routes the scroll wheel, and the terminal modes behind that decision: `wheel_routing` (`mouse_report`, `alternate_scroll` or `host_scroll`) plus `mouse_reporting`, `alternate_screen` and `mouse_alternate_scroll`. Recomputed per request, so it reflects what the application has enabled now rather than when it started, which is what intermittent scrolling problems need.
- Added `toggle_space_highlight` and `toggle_agent_highlight` keybinds, marking a space or agent so it renders in a distinct colour in the sidebar. Each press steps the mark down a level — pink, purple, blue, then a parked grey that reads quieter than an unmarked row, then off — so one press still means "this is the thing I care about". A mark reads the same from either list, taking the louder of the space's and the agent's level, and is saved with the session. Colours come from `ui.sidebar_highlight_colors = [background, low, medium, high]`, where an empty entry keeps the default; the older `ui.sidebar_highlight_color` still sets the top level. Unbound by default.
- Added `remote.keep_offline_mirrors`, deciding what mirrors of an unreachable host do before anyone touches the keybind: hide until the host answers, which is still the default, or stay in place greyed. `toggle_offline_mirrors` flips it for the running session, and that answer is kept with the session and is not undone by a config reload.
...
- Added `herdr focus <agent|space|pane> [--observe] [--takeover] [--host TARGET]`, which opens a client showing just that one thing, independent of what any other client is focused on. Resolves an agent name, a space id or label, or a pane id; `--observe` is read-only. A space shows its focused pane. Run it bare to list what can be focused. `--host` runs the whole thing on another machine over ssh, resolving herdr's path there and allocating a pty. When an attach is refused because something already holds the terminal, which on a mirrored host is the mirror, it now suggests observing or driving it from the mirroring machine rather than only offering `--takeover`.
...
- Added a `remote_host` sidebar token with a stable per-host colour (overridable with a `color` per host), so mirrored machines read distinctly from workspace names.
- Added a `mirror_all` option to `[[remote.spaces]]` to mirror every remote space, not only those hosting an agent.
...
### Fixed
- The Linux server now raises its own file-descriptor limit to 65536 at startup, as the macOS one already did. The function that should have done it had an empty body, so a Linux server ran on the 1024 default: mirroring costs about 14.5 descriptors per pane, which put the ceiling near 70 panes on a machine with 64 GB of memory and eleven idle cores, and hitting it makes the server unreachable rather than slow.
- The server now says how many API connections it is holding, every 25 either way, and names the peer whenever a connection outlives the process that opened it. A server can leak connections for a day while looking perfectly healthy; the first sign used to be `accept` failing, by which point it was too late to see what had caused it.
- The API socket no longer stops answering for the rest of a server's life after one failed accept. Running out of file descriptors made `accept` fail, and the listener treated that as fatal and exited, leaving herdr running but unreachable: no CLI, no client, and no way to stop it or hand it over, since both of those are themselves API calls. A failed accept is now logged and retried.
- Connections whose opener has exited are closed instead of being held for the life of the server. A socket outlives its opener whenever something else still holds the far end -- a child that inherited it, an ssh session that has not finished dying -- and such a connection never reads as closed, so it kept a descriptor and a thread indefinitely. On one host that reached 837 of them and exhausted the process's descriptors. The peer's recorded pid is checked as well as the stream; a recycled pid reads as alive, so this errs towards keeping a connection rather than closing a live one.
- Two hosts can now mirror each other without either seeing its own agents twice. A host reports our panes back to us as its mirrors, and only reflections of a *third* host we also mirror were dropped, so our own work came home wearing that host's name and sat beside itself in the sidebar. A reflection is now recognised as ours by the terminal it names rather than by the host name attached to it, since a name is only what the other machine happens to call us -- `pandora`, `ryi@pandora`, a tailnet address -- while the terminal is either one of ours or it is not.
- Reading a mirror no longer un-reads itself on the next poll. A host reports "done" -- idle with output nobody has read -- every time it is asked, until someone reads it *on that host*; the mirror applied that answer on each poll, so marking one read here was undone thirty seconds later with nothing having changed. An unread mark is now laid down once per remote change: the same report repeated leaves the mirror as the reader left it, and only newer output marks it unread again. A host too old to stamp when its agents last changed keeps the previous behaviour, since it cannot be told apart from one repeating itself.
- A mirror reached through another host now shows the machine actually running it, not the hop it arrived through. Reaching a fleet through one hub used to put every agent under the hub's name and colour, which reads as though one machine were running everything. The reporting host passes on how it labels each origin, so the name and colour match what that machine is called everywhere else; a host too old to pass one on falls back to the host part of its target, and the sidebar derives its usual colour from that. Only the display changes -- the mirror is still reconciled against the host being polled.
- Mirroring two hosts that mirror each other no longer shows their agents twice. A mirror is an ordinary local space to the host holding it, so a third machine discovering that host used to pick up its reflections as if they were its own agents. Panes now say what they are a mirror of, and a reflection is dropped when it names a host we mirror ourselves, or when the pane it reflects is right beside it in the same host's answer. A reflection of a host that is *not* mirrored locally is kept, since it is the only way that agent arrives at all. Hosts spelled differently by different machines -- `alleria` against `ryielle@alleria` -- are recognised as one machine.
- Mouse clicks now reach the pane behind a direct terminal attach, which is what a mirrored space is. An attach enables mouse capture on its own terminal whatever it attached to, because that is the only way to get the wheel, and it turned every mouse event that was not a wheel notch into a scroll action it then discarded — so a click, drag or motion over a mirrored space was dropped before it was ever sent. Non-wheel mouse events are forwarded now, and the server drops the ones the attached pane never asked for, since it is the end that knows which modes the pane has set. Both machines need this build: the fix spans the attach client on one and the server on the other.
- A new tab asked for on a mirrored space is now created on the machine that space belongs to, not on the machine herdr is running on. It arrives as another mirror of the same remote space, beside the one it was asked from, since a mirror stands for one remote pane. Tabs made locally inside a mirror also disappeared without warning: a mirror's whole workspace is closed when its remote pane goes away, and mirrors are rebuilt on every reconnect and handoff, so the shell went with it. Only the pane the user asked for is mirrored; the other shells in that remote space stay out of the sidebar as before.
- The `number` sidebar token now holds its column when an entry has no switch number. Only the first nine get one, so it used to drop out entirely and pull every row past the ninth two columns left, which is the opposite of what a number column is for.
- The client now repairs mouse reporting when its terminal has gone silent: if capture should be on but no mouse report has arrived for 15 seconds, the mode is turned off and on again. Rewriting it was not enough, because a session-resuming layer such as mosh models terminal state and sends only differences, so asking for a mode it already believes is set produces nothing on the wire. A reconnecting mosh session repaints from its own screen model, which does not carry DEC private modes, so the terminal quietly stopped sending mouse reports and nothing ever put the mode back: scrolling did nothing at all, with no wheel event reaching herdr to route.
- Mirrored agents are now aged by when they changed state on their own host, rather than by when the mirror was built. Applying a remote's state locally restamped it as changing now, and mirrors are rebuilt on every reconnect and handoff, so every mirrored agent collapsed into the newest idle group however long it had really sat. The remote's state-change time travels with the pane; a remote that does not report one leaves the age unknown instead of guessing.
